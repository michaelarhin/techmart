  ...
  <div className="pt-24 md:pt-28 pb-10">
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show" className="surface rounded-4xl p-7 sm:p-10">
        ...
        <h1 className="mt-2 text-3xl sm:text-5xl font-extrabold text-ink leading-tight">{title}</h1>
        ...
        >
          {sections.map((s, i) => (
            <motion.section
              ...
              id={s.id}
              variants={item}
              className="surface rounded-3xl p-6 sm:p-7 scroll-mt-28"
            >
              <h2 className="text-lg sm:text-xl font-bold text-ink flex items-baseline gap-3">
                <span className="text-accent text-sm font-extrabold">{String(i + 1).padStart(2, '0')}</span>
                {s.heading}
              </h2>
